...
public func == (left: Scale, right: Scale) -> Bool {
  return left.key == right.key && left.type == right.type && left.direction == right.direction
}

public enum ScaleDirection: String, Codable
{
...
public struct Scale: Equatable, Codable {
  ...
  ///   - key: Key of scale being initilized.
	public init(type: ScaleType, key: Key, direction: ScaleDirection) {
    self.type = type
    self.key = key
